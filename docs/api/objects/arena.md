...
relative to the arena, and the arena can be resized and moved.
...
- **number** `Arena.x` (readonly) - the x position of the center of the Arena,
  after resizing.
- **number** `Arena.y` (readonly) - the position of the bottom of the Arena, after
...
- **number** `Arena.currentx` (readonly) - the *current* x of the arena in pixels.
  Differs from `x` in that it will accurately reflect the arena position in the middle of
  moving, too.
- **number** `Arena.currenty` (readonly) - the *current* y of the arena in pixels.
...
- **{table of number, number, number, number = 1}** `Arena.innerColor` - Set RGBA
  color (0-1) of Arena's inner sprite.
- **{table of number, number, number, number = 255}** `Arena.innerColor32` - Set
  RGBA color (0-255) of Arena's inner sprite.
- **{table of number, number, number, number = 1}** `Arena.outerColor` - Set RGBA
  color (0-1) of Arena's outer sprite.
- **{table of number, number, number, number = 255}** `Arena.outerColor32` - Set
  RGBA color (0-255) of Arena's outer sprite.
- **boolean** `Arena.isResizing` (readonly) - Tells you if the Arena is currently
  being resized.

  : There's an equivalent of this in 0.2.1a, just use
  `Arena.currentwidth ~= Arena.width or Arena.currentheight ~= Arena.height`.
- **boolean** `Arena.isMoving` (readonly) - Tells you if the Arena is currently
  being moved.
- **boolean** `Arena.isModifying` (readonly) - Returns true if the Arena is being
...
- `Arena.Move(number x, number y, boolean movePlayer = true, boolean immediate = false)` -
  ...
  immediately.
- `Arena.MoveTo(number x, number y, boolean movePlayer = true, boolean immediate = false)` -
...
- `Arena.MoveAndResize(number x, number y, number width, number height, boolean movePlayer = true, boolean immediate = false)` -
...
- `Arena.MoveToAndResize(number x, number y, number width, number height, boolean movePlayer = true, boolean immediate = false)` -
...
- New in v0.6.6. `Arena.Hide(boolean showWhenWaveEnds = true)` - Makes the Arena
